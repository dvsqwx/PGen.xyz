...
if (canvas) {
  ...
  scene.add(key);
  const fill = new THREE.DirectionalLight(0xcfd6ff, 0.32);
  fill.position.set(-4, 2, -2);
  scene.add(fill);
  const rim = new THREE.PointLight(0xffffff, 0.60, 30);
  rim.position.set(0, 1.2, 3.6);
  scene.add(rim);

  const lime = new THREE.PointLight(0xb9ff2a, 0.28, 30);
  lime.position.set(-2.4, 0.4, 2.8);
  scene.add(lime);
  ...
  logo.add(new THREE.Mesh(tetraGeo, faceMat));
  ...
  const limeEdges = new THREE.LineSegments(
    new THREE.EdgesGeometry(tetraGeo),
    new THREE.LineBasicMaterial({ color: 0xb9ff2a, transparent: true, opacity: 0.22 })
  );
  ...
  const dotMat = new THREE.MeshStandardMaterial({ color: 0xffffff, metalness: 0.08, roughness: 0.45 });
  const dotPositions = [
    [ 0.95,  0.25,  0.20],
    [-0.90, -0.05,  0.35],
    [ 0.10, -0.85,  0.10],
